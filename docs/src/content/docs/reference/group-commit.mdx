...
committy --non-interactive group-commit --mode apply --output json \
  [--include-unstaged] [--auto-stage] [--push --confirm-push] [--git-config key=value]
```
...
- `--confirm-push`: required whenever `--push` is used.
- `--git-config key=value`: repeatable native Git override for apply-mode git subprocesses.
